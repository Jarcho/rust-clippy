...
fn collect_unwrap_info<'tcx>(
    ...
    fn is_relevant_option_call(cx: &LateContext<'_>, ty: Ty<'_>, method_name: Symbol) -> bool {
        cx.is_diag_item(ty.ty_adt_def(), sym::Option) && matches!(method_name, sym::is_none | sym::is_some)
    }

    fn is_relevant_result_call(cx: &LateContext<'_>, ty: Ty<'_>, method_name: Symbol) -> bool {
        cx.is_diag_item(ty.ty_adt_def(), sym::Result) && matches!(method_name, sym::is_err | sym::is_ok)
    }
    ...
    match expr.kind {
        ...
        ExprKind::MethodCall(method_name, receiver, [], _)
            if let Some(local_id) = receiver.path_local_id()
                ...
                && (is_relevant_option_call(cx, ty, name) || is_relevant_result_call(cx, ty, name)) =>
        {
            ...
            let kind = if cx.is_diag_item(ty.ty_adt_def(), sym::Option) {
    ...
    fn visit_expr(&mut self, expr: &'tcx Expr<'_>) {
        ...
        if let Some(higher::If { cond, then, r#else }) = higher::If::hir(expr) {
            ...
            if let ExprKind::MethodCall(method_name, self_arg, ..) = expr.kind
                ...
                && let Some(id) = self_arg.path_local_id()
