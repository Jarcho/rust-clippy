...
fn lint_map_unit_fn(
    ...
    let (map_type, variant, lint) = match cx.opt_diag_name(cx.typeck_results().expr_ty(var_arg)) {
        Some(sym::Option) => ("Option", "Some", OPTION_MAP_UNIT_FN),
        Some(sym::Result) => ("Result", "Ok", RESULT_MAP_UNIT_FN),
        _ => return,
    };
