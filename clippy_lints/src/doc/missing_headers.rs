...
pub fn check(
    ...
    if !headers.errors {
        if cx.is_diag_item(return_ty(cx, owner_id), sym::Result) {
            span_lint(
                cx,
                MISSING_ERRORS_DOC,
                span,
            ...
            && cx.is_diag_item(subs.as_coroutine().return_ty(), sym::Result)
        {
            span_lint(
                cx,
                MISSING_ERRORS_DOC,
...
fn find_panic(cx: &LateContext<'_>, body_id: BodyId) -> Option<Span> {
    ...
    for_each_expr(cx, cx.tcx.hir_body(body_id), |expr| {
        ...
        if let Some(arglists) =
            ...
            && matches!(cx.opt_diag_name(receiver_ty), Some(sym::Option | sym::Result))
            && !fulfill_or_allowed(cx, MISSING_PANICS_DOC, [expr.hir_id])
            && panic_span.is_none()
        {
