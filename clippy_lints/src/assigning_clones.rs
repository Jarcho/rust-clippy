...
use clippy_utils::{is_in_test, last_path_segment, local_is_initialized, path_to_local, sym};
            ...
            && let (call_kind, fn_name, fn_def, fn_arg, fn_gen_args) = match rhs.kind {
                ExprKind::Call(f, [arg])
                    if let ExprKind::Path(qpath) = &f.kind
                        && let Some(def) = typeck.path_def((qpath, f.hir_id)) =>
                {
                    (CallKind::Ufcs, last_path_segment(qpath).ident.name, def, arg, typeck.node_args(f.hir_id))
                },
                ExprKind::MethodCall(name, recv, [], _)
                if let Some(def) = typeck.type_dependent_def(rhs.hir_id) => {
                    (CallKind::Method, name.ident.name, def, recv, typeck.node_args(rhs.hir_id))
                },
                _ => return,
            }
            && matches!(fn_name, sym::clone | sym::to_owned)
            && let Some(diag_name) = cx.assoc_parent_diag_name(fn_def)
            && let which_trait = match (fn_name, diag_name) {
                (sym::clone, sym::Clone) => CloneTrait::Clone,
                (sym::to_owned, sym::ToOwned) if self.msrv.meets(cx, msrvs::CLONE_INTO) => CloneTrait::ToOwned,
                _ => return,
            }
