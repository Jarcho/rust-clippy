...
fn parse_repeat_arg(cx: &LateContext<'_>, e: &Expr<'_>) -> Option<RepeatKind> {
    if let ExprKind::Lit(lit) = &e.kind {
        ...
        if cx.is_lang_item(ty, LangItem::String)
            || (cx.is_lang_item(ty, LangItem::OwnedBox) && get_ty_param(ty).is_some_and(Ty::is_str))
...
pub(super) fn check(
    ...
    if let ExprKind::Call(repeat_fn, [repeat_arg]) = take_self_arg.kind
        ...
        && cx.is_lang_item(cx.typeck_results().expr_ty(collect_expr), LangItem::String)
