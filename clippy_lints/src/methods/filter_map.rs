...
impl<'tcx> OffendingFilterExpr<'tcx> {
        ...
        match *self {
            ...
            | OffendingFilterExpr::IsOk {
                ...
                if let ExprKind::MethodCall(seg, map_arg, ..) = map_body.value.kind
                    ...
                    && let simple_equal = (receiver.is_path_local(filter_param_id)
                        && map_arg_peeled.is_path_local(map_param_id))
                    && let eq_fallback = (|a: &Expr<'_>, b: &Expr<'_>| {
                        ...
                        a_path.is_path_local(filter_param_id)
                            && b.is_path_local(map_param_id)
            ...
            OffendingFilterExpr::Matches { variant_def_id } => {
                let expr_uses_local = |pat: &Pat<'_>, expr: &Expr<'_>| {
                    if let PatKind::TupleStruct(QPath::Resolved(_, path), [subpat], _) = pat.kind
                        ...
                        && expr.peel_blocks().is_path_local(local_id)
                ...
                if scrutinee.is_path_local(map_param_id)
    ...
    fn hir(cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>, filter_param_id: HirId) -> Option<Self> {
            ...
            && let Some(recv_ty) = cx.typeck_results().expr_ty(receiver).peel_refs().ty_adt_def()
        {
            ...
            && scrutinee.is_path_local(filter_param_id)
...
fn is_filter_some_map_unwrap(
    ...
    let iterator = cx.is_type_dependent_assoc_of_diag_item(expr, sym::Iterator);
    let option = cx.is_diag_item(cx.typeck_results().expr_ty(filter_recv), sym::Option);
...
fn is_find_or_filter<'a>(
    ...
    if cx.is_type_dependent_assoc_of_diag_item(map_recv, sym::Iterator)
        // filter(|x| ...is_some())...
