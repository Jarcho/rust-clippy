...
pub(super) fn check(
    ...
    if !cx.is_diag_item(option_ty, sym::Option) {
        return;
    }
        ...
        hir::ExprKind::Closure(&hir::Closure { body, .. }) => {
            ...
            match &closure_expr.kind {
                hir::ExprKind::MethodCall(_, receiver, [], _) => {
                    if receiver.is_path_local(closure_body.params[0].pat.hir_id)
                        && let adj = cx
                            .typeck_results()
                ...
                hir::ExprKind::AddrOf(hir::BorrowKind::Ref, m, inner) if same_mutability(m) => {
                    ...
                    {
                        inner2.is_path_local(closure_body.params[0].pat.hir_id)
                    } else {
                        false
                    }
                },
