    ...
    fn check_methods<'tcx>(&self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'_>) {
        ...
        if let Some((name, recv, args, span, call_span)) = method_call(expr) {
            match (name, args) {
                ...
                (sym::collect, []) if cx.is_type_dependent_assoc_of_diag_item(expr, sym::Iterator) => {
                ...
                (sym::count, []) if cx.is_type_dependent_assoc_of_diag_item(expr, sym::Iterator) => {
                    match method_call(recv) {
                        Some((sym::cloned, recv2, [], _, _)) => {
                            iter_overeager_cloned::check(
                                cx,
                ...
                (sym::try_into, []) if cx.is_type_dependent_assoc_of_diag_item(expr, sym::TryInto) => {
    ...
    fn matches<'a>(self, cx: &LateContext<'a>, parent_ty: Ty<'a>, ty: Ty<'a>) -> bool {
        fn matches_value<'a>(cx: &LateContext<'a>, parent_ty: Ty<'a>, ty: Ty<'a>) -> bool {
            if ty == parent_ty {
                ...
                && matches!(cx.opt_diag_name(adt), Some(sym::Rc | sym::Arc))
            {
