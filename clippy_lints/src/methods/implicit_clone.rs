...
use clippy_utils::{peel_middle_ty_refs, sym};
...
pub fn check(cx: &LateContext<'_>, method_name: Symbol, expr: &hir::Expr<'_>, recv: &hir::Expr<'_>) {
    if let Some(method_parent_id) = cx.assoc_parent_id(cx.type_dependent_def(expr.hir_id))
        ...
        && !(ref_count > 0 && cx.is_diag_item(method_parent_id, sym::ToOwned))
...
pub fn is_clone_like(cx: &LateContext<'_>, method_name: Symbol, method_parent_id: hir::def_id::DefId) -> bool {
    match method_name {
        sym::to_os_string => cx.is_impl_for_diag_ty(method_parent_id, sym::OsStr),
        sym::to_owned => cx.is_diag_item(method_parent_id, sym::ToOwned),
        sym::to_path_buf => cx.is_impl_for_diag_ty(method_parent_id, sym::Path),
        sym::to_string => cx.is_diag_item(method_parent_id, sym::ToString),
        sym::to_vec => cx
            .opt_impl_ty(method_parent_id)
            .is_some_and(|ty| ty.skip_binder().is_slice()),
        _ => false,
    }
}
