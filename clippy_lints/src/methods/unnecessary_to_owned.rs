...
use clippy_utils::{fn_def_id, get_parent_expr, is_expr_temporary_value, peel_middle_ty_refs, return_ty, sym};
...
pub fn check<'tcx>(
    ...
    if let Some(method_parent_id) = cx.assoc_parent_id(cx.type_dependent_def(expr.hir_id))
        && args.is_empty()
    {
...
fn check_into_iter_call_arg(
    ...
    if let Some(parent) = get_parent_expr(cx, expr)
        ...
        && !cx.is_diag_item( cx.typeck_results().expr_ty(receiver), sym::Cow)
...
fn check_split_call_arg(cx: &LateContext<'_>, expr: &Expr<'_>, method_name: Symbol, receiver: &Expr<'_>) -> bool {
        ...
        && let Some(arg_snippet) = argument_expr.span.get_source_text(cx)
    {
        ...
        let as_ref = if cx.is_lang_item(cx.typeck_results().expr_ty(expr), LangItem::String)
...
fn is_cloned_or_copied(cx: &LateContext<'_>, method_name: Symbol, method_parent_id: DefId) -> bool {
    matches!(method_name, sym::cloned | sym::copied) && cx.is_diag_item(method_parent_id, sym::Iterator)
}
...
fn is_cow_into_owned(cx: &LateContext<'_>, method_name: Symbol, method_parent_id: DefId) -> bool {
    method_name == sym::into_owned && cx.is_impl_for_diag_ty(method_parent_id, sym::Cow)
}
...
fn is_to_string_on_string_like<'a>(
    ...
    if method_name != sym::to_string || !cx.is_diag_item(method_parent_id, sym::ToString) {
        return false;
    }
...
fn is_str_and_string(cx: &LateContext<'_>, arg_ty: Ty<'_>, original_arg_ty: Ty<'_>) -> bool {
    original_arg_ty.is_str() && cx.is_lang_item(arg_ty, LangItem::String)
}
...
fn is_slice_and_vec(cx: &LateContext<'_>, arg_ty: Ty<'_>, original_arg_ty: Ty<'_>) -> bool {
    (original_arg_ty.is_slice() || original_arg_ty.is_array() || original_arg_ty.is_array_slice())
        && cx.is_diag_item(arg_ty, sym::Vec)
}
