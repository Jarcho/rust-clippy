...
use rustc_span::{
    BytePos, DUMMY_SP, Pos, RelativeBytePos, SourceFile, SourceFileAndLine, Span, SpanData, SyntaxContext, hygiene,
};
...
pub trait SpanExt: SpanLike {
    ...
    fn map_range<'sm>(
        ...
        let range = SourceFileRange::new(sm.source_map(), data.lo..data.hi)
            .as_mut()
            .and_then(f)?
            .source_range();
...
fn dbg_check_range(current: &SourceFileRange<'_>, new: Range<RelativeBytePos>) {
    ...
    let overlapping = new.start > new.end;
    let exceeds = new.end > current.file().source_len;
    let non_utf8 = current
        .file_text()
        .get(new.start.to_usize()..new.end.to_usize())
        .is_none();
    if overlapping || exceeds || non_utf8 {
        let file = current.file();
        let mut msg = String::with_capacity(512);
        let _ = write!(msg, "error setting the requested range `{new:?}");
        if overlapping {
            msg.push_str("\n  the start and end overlap");
        }
        if exceeds {
            let _ = write!(
                msg,
                "\n  the range exceed the length of the current file ({})",
                file.source_len.to_u32(),
            );
        }
        if non_utf8 {
            msg.push_str("\n  the range does not lie on a UTF-8 boundary");
        }
        ...
        if new.start != current.range.start {
            print_loc(
                "new start",
                BytePos::from_u32(new.start.to_u32().wrapping_add(file.start_pos.to_u32())),
            );
        }
        if new.end != current.range.end {
            print_loc(
                "new end",
                BytePos::from_u32(new.end.to_u32().wrapping_add(file.start_pos.to_u32())),
            );
        }
...
impl<'sm> SourceFileRange<'sm> {
    ...
    pub fn set_lo(&mut self, pos: BytePos) -> &mut Self {
        ...
        dbg_check_range(self, rel_pos..self.range.end);
    ...
    pub fn set_hi(&mut self, pos: BytePos) -> &mut Self {
        let rel_pos = RelativeBytePos::from_u32((pos - self.file().start_pos).to_u32());
        dbg_check_range(self, self.range.start..rel_pos);
...
/// Checks if the last token is the string is either a line comment or an incomplete token.
